...
std::string getLocalIPAddress()
{
    try
    {
        ...
        boost::asio::ip::tcp::resolver::results_type endpoints = resolver.resolve( boost::asio::ip::host_name(), "" );
...
public:
    BitswapNode( const std::string &name, int port ) : node_name_( name ), port_( port ), is_running_( false )
    {
        initializeNode();
    }
    ...
    void initializeNode()
    {
        ...
        std::shared_ptr<libp2p::crypto::CryptoProvider> crypto_provider =
            std::make_shared<libp2p::crypto::CryptoProviderImpl>( csprng,
                                                                  ed25519_provider,
                                                                  rsa_provider,
                                                                  ecdsa_provider,
                                                                  secp256k1_provider,
        ...
        auto protocols = libp2p::protocol::factory::ProtocolFactory::createProtocols( host_,
                                                                                      protocol_config,
                                                                                      injector );
        protocols.identify->start();

        // Bind Listen Address
        auto bindaddress = ( boost::format( "/ip4/%s/tcp/%d/p2p/%s" ) % "0.0.0.0" % port_ % host_->getId().toBase58() )
                               .str();
    ...
    libp2p::peer::PeerInfo getPeerInfo() const
    {
        ...
        auto peer_addr_str = ( boost::format( "/ip4/%s/tcp/%d/p2p/%s" ) % local_ip % port_ % host_->getId().toBase58() )
                                 .str();
        ...
        if ( !ma_res )
        {
            ...
            peer_addr_str =
                ( boost::format( "/ip4/%s/tcp/%d/p2p/%s" ) % "127.0.0.1" % port_ % host_->getId().toBase58() ).str();
...
# ----------------
    ...
    bool setupNodes()
    {
        ...
        if ( server_peer_info.id == client_peer_info.id )
        {
            ...
            std::cerr << "[ERROR] This will prevent connection. The libp2p key generation might be deterministic."
                      << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "[WARNING] Failed to add server peer info to client: " << add_result.error().message()
                      << std::endl;
        }
    ...
    bool publishTestContent()
    {
        ...
        try
        {
            ...
            server_node_->getBitswap()->PublishDirectory(
                ...
                [&]( libp2p::outcome::result<CID> result )
                {
                    ...
                    else
                    {
                        ...
                        if ( cid_str_result )
                        {
                            std::cout << "[SUCCESS] Directory structure published with CID: " << cid_str_result.value()
                                      << std::endl;
                        }
                        else
                        {
            ...
            std::cout << "[PUBLISH] Complex directory structure successfully published and available for retrieval"
                      << std::endl;
            return true;
        }
        catch ( const std::exception &e )
    ...
    bool verifyRetrievedContent( const UnixFSContent &content )
    {
        ...
        {
            std::cerr << "[ERROR] Expected directory or multi-file archive, got type: "
                      << static_cast<int>( content.type ) << std::endl;
            return false;
        }
        ...
        for ( const auto &expected_file : test_files_ )
        {
            ...
            // Check content
            if ( std::memcmp( retrieved_file->content.data(),
                              expected_file.content.data(),
                              expected_file.content.size() ) != 0 )
            {
            ...
            std::cout << "[OK] ✅ " << expected_file.path << " (" << retrieved_file->content.size() << " bytes)"
                      << std::endl;
        }
        ...
        if ( all_verified )
        {
            ...
            for ( const auto &expected_file : test_files_ )
            {
                std::cout << "[SUCCESS]   " << expected_file.path << " (" << expected_file.content.size() << " bytes)"
                          << std::endl;
            }
            return true;
        }
    ...
    void run()
    {
        ...
        try
        {
            ...
            if ( success )
            {
                std::cout << "\\n[COMPLETE] ✅ All tests passed! Bitswap server-client communication working!"
                          << std::endl;
            }
            else
            {
    ...
    bool testIndividualFileRetrieval()
    {
        ...
        try
        {
            ...
            std::cout << "[SETUP] Created test file: " << temp_file << " (" << file_content.size() << " bytes)"
                      << std::endl;
            ...
            server_node_->getBitswap()->PublishFile(
                temp_file,
                [&]( libp2p::outcome::result<CID> result )
                {
                    if ( result.has_value() )
            ...
            {
                std::cerr << "[ERROR] Expected single file, got type: " << static_cast<int>( retrieved_content.type )
                          << std::endl;
                return false;
            }
            ...
            std::cout << "[VERIFY] ✅ Individual file content verified (" << file.content.size() << " bytes)"
                      << std::endl;
